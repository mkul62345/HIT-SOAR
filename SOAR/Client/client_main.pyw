...
def push_logs():
    ...
    DIRECTORY_PATH = "S:\\HIT-SOAR\\SOAR\\Client" 
    dir = os.listdir( DIRECTORY_PATH + "\\Logs" ) #%SystemRoot%\System32\winevt\Logs folder
    for file in dir:

        #If file is an event log file
        if file.find(".evtx") != -1: 
            parser = PyEvtxParser(DIRECTORY_PATH + "\\Logs\\" + file)
